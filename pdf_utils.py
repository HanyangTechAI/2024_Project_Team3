...
def extract_image_from_pdf(pdf_path, page_number, problem_number, output_folder):
    """
    PDF의 특정 페이지에서 문제 번호에 해당하는 이미지를 추출.
    """
    try:
        pdf_document = fitz.open(pdf_path)
        ...
        for block in blocks:
            for line in block["lines"]:
                for span in line["spans"]:
                    if f"{problem_number}." in span["text"] or f"Q{problem_number}" in span["text"]:
